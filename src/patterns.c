...
/**
 * Pattern to make small movement, pause for 10 seconds, and repeat 1-4 more
 * times
 * @param step_idx Pointer to the current step index
 */
...
void pattern_03(int *step_idx) {
  ...
  for (int i = 0; i < reps; i++) {
    int rep_length = rand() % (STEPS_PER_REV + 1); // will go a random number of steps
                                                   // (always less than a full revolution)
    step_n_times(step_idx, direction,
                 rep_length); // move motor forward or backward
